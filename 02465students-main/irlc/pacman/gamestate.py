...
class GameState:
    ...
    *this* class. So for instance to know if the game is won or lost you can do:
    ...
    def players(self):
        ...
        """
        return self._unsafe_getNumAgents()
    ...
    def A(self):
        ...
        if self.is_won() or self.is_lost():
            ...
            return self._unsafe_getLegalActions(self.player())
    ...
    def f(self, a : str) -> object:
        ...
        suc = self._unsafe_generateSuccessor(self.player(), a)
        suc._player = (self.player() + 1) % self._unsafe_getNumAgents()
        return suc
    ...
    explored = set()
    def _unsafe_getAndResetExplored():
        tmp = GameState.explored.copy()
        GameState.explored = set()
        return tmp

    _unsafe_getAndResetExplored = staticmethod(_unsafe_getAndResetExplored)

    def _unsafe_getLegalActions(self, agentIndex=0):
        # """
    ...
    def _unsafe_generateSuccessor(self, agentIndex, action):
        # """
        ...
        if agentIndex == 0:  # Pacman is moving
            state.data._eaten = [False for i in range(state._unsafe_getNumAgents())]
    ...
    def _unsafe_getLegalPacmanActions(self):
        return self._unsafe_getLegalActions(0)

    def _unsafe_generatePacmanSuccessor(self, action):
        # """
        # Generates the successor state after the specified pacman move
        # """
        return self._unsafe_generateSuccessor(0, action)

    def _unsafe_getPacmanState(self):
        # """
    ...
    def _unsafe_getPacmanPosition(self):
        return self.data.agentStates[0].getPosition()

    def _unsafe_getGhostStates(self):
        return self.data.agentStates[1:]

    def _unsafe_getGhostState(self, agentIndex):
        if agentIndex == 0 or agentIndex >= self._unsafe_getNumAgents():
            raise Exception("Invalid index passed to getGhostState")
        return self.data.agentStates[agentIndex]

    def _unsafe_getGhostPosition(self, agentIndex):
        if agentIndex == 0:
            raise Exception("Pacman's index passed to getGhostPosition")
        return self.data.agentStates[agentIndex].getPosition()

    def _unsafe_getGhostPositions(self):
        return [s.getPosition() for s in self._unsafe_getGhostStates()]

    def _unsafe_getNumAgents(self):
        return len( self.data.agentStates )

    def _unsafe_getScore(self):
        return float(self.data.score)

    def _unsafe_getCapsules(self):
        # """
        # Returns a list of positions (x,y) of the remaining capsules.
        # """
        return self.data.capsules

    def _unsafe_getNumFood(self):
        return self.data.food.count()

    def _unsafe_getFood(self):
        # """
    ...
    def _unsafe_hasFood(self, x, y):
        return self.data.food[x][y]

    def _unsafe_hasWall(self, x, y):
...
class ClassicGameRules:
    ...
    def getProgress(self, game):
        return float(game.state._unsafe_getNumFood()) / self.initialState._unsafe_getNumFood()
...
class PacmanRules:
    ...
    def getLegalActions( state ):
        ...
        """
        return Actions.getPossibleActions(state._unsafe_getPacmanState().configuration, state.data.layout.walls)
    ...
    def consume( position, state ):
        ...
        if state.data.food[x][y]:
            ...
            # TODO: cache numFood?
            numFood = state._unsafe_getNumFood()
            if numFood == 0 and not state.data._lose:
                state.data.scoreChange += 500
                state.data._win = True
        # Eat capsule
        if( position in state._unsafe_getCapsules()):
...
class GhostRules:
    ...
    def getLegalActions( state, ghostIndex ):
        ...
        """
        conf = state._unsafe_getGhostState(ghostIndex).configuration
    ...
    def checkDeath( state, agentIndex):
        pacmanPosition = state._unsafe_getPacmanPosition()
...
def runGames( layout, pacman, ghosts, display, numGames, record, numTraining = 0, catchExceptions=False, timeout=30 ):
    ...
    if (numGames-numTraining) > 0:
        scores = [game.state._unsafe_getScore() for game in games]
